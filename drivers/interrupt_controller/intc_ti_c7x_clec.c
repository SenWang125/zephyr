...
static inline void c7x_epri_set(uint8_t local_irq, uint8_t priority)
{
	 ...
	 * z_c7x_irq_priority_set() does. Masking turns 0 or 8 into EPRI 0, highest.
	 */
	unsigned int p = (priority > 7U) ? 7U : (priority < 1U) ? 1U : priority;

	z_c7x_write_epri((unsigned int)local_irq, p << 5U);
}
...
static inline void c7x_efclr(uint8_t local_irq)
{
	 ...
	 * __EPRI above.
	 */
	z_c7x_write_efclr(UINT64_C(1) << (local_irq & 0x3FU));
}
