...
#define ARCH_IRQ_CONNECT(irq_n, prio, isr_fn, isr_arg, flags)                                      \
	{                                                                                          \
		...
		c7x_irq_priority_set((irq_n), (prio));                                           \
	}
...
static ALWAYS_INLINE void c7x_irq_priority_set(unsigned int irq, unsigned int prio)
{
	...
	write_epri((unsigned int)idx, epri_val);
}

/*
 * All 64 events share one vector, therefore a direct ISR still runs through the common entry
 * and ISR table.
 */
#define ARCH_IRQ_DIRECT_CONNECT(irq_p, priority_p, isr_p, flags_p)                                 \
	{                                                                                          \
		Z_ISR_DECLARE_DIRECT((irq_p), ISR_FLAG_DIRECT, isr_p);                             \
		c7x_irq_priority_set((irq_p), (priority_p));                                     \
	}
