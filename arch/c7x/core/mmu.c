...
#define C7X_MMU_POOL_BYTES	(C7X_MMU_POOL_ENTRIES * sizeof(uint64_t))
...
/* Page table pool in the image's own .noinit (DDR), 4 KB aligned. */
...
{
	uint32_t slot = z_c7x_mmu_get_next_slot();
	uint64_t *base;
	uint32_t i;
	...
	base = z_c7x_mmu_get_tables_base();
	z_c7x_mmu_set_next_slot(slot + 1U);

	base += (uint32_t)(slot * C7X_MMU_ENTRIES);
...
static volatile uint32_t z_c7x_mmu_fail_stat[2] __used __aligned(64) Z_GENERIC_SECTION(.bss:c7x_diag);
...
static void z_c7x_mmu_map_block(uint64_t *l0, uint64_t va, uint64_t pa,
						  uint64_t attr)
{
	...
	e = z_c7x_mmu_read_entry(l0, i0);
	if ((e & C7X_MMU_DESC_TYPE_MASK) == C7X_MMU_DESC_TABLE) {
		l1 = (uint64_t *)(uintptr_t)(e & ~C7X_MMU_PAGE_MASK);
	} else {
		l1 = z_c7x_mmu_alloc_table();
		if (!l1) {
			z_c7x_mmu_fail(5U, va);
		}
		z_c7x_mmu_write_entry(l0, i0, (uint64_t)(uintptr_t)l1 | C7X_MMU_DESC_TABLE);
	}

	e = z_c7x_mmu_read_entry(l1, i1);
	if ((e & C7X_MMU_DESC_TYPE_MASK) == C7X_MMU_DESC_TABLE) {
		l2 = (uint64_t *)(uintptr_t)(e & ~C7X_MMU_PAGE_MASK);
	} else {
		l2 = z_c7x_mmu_alloc_table();
		if (!l2) {
			z_c7x_mmu_fail(6U, va);
		}
		z_c7x_mmu_write_entry(l1, i1, (uint64_t)(uintptr_t)l2 | C7X_MMU_DESC_TABLE);
	}

	z_c7x_mmu_write_entry(l2, i2, (pa & ~C7X_MMU_BLOCK_MASK) | attr);
}

static uint64_t *z_c7x_mmu_walk_l2(uint64_t *l0, uint64_t va)
{
	uint32_t i0 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(0)) & C7X_MMU_INDEX_MASK);
	uint32_t i1 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(1)) & C7X_MMU_INDEX_MASK);
	uint64_t *l1, *l2;
...
static void z_c7x_mmu_map_page_l3(uint64_t *l0, uint64_t va, uint64_t pa, uint64_t attr)
{
	uint32_t i2 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(2)) & C7X_MMU_INDEX_MASK);
	...
	l2 = z_c7x_mmu_walk_l2(l0, va);
	if (!l2) {
		z_c7x_mmu_fail(2U, va);
	}

	e = z_c7x_mmu_read_entry(l2, i2);
	if ((e & C7X_MMU_DESC_TYPE_MASK) == C7X_MMU_DESC_TABLE) {
		l3 = (uint64_t *)(uintptr_t)(e & ~C7X_MMU_PAGE_MASK);
	} else {
	...
	z_c7x_mmu_write_entry(l3, i3, (pa & ~C7X_MMU_PAGE_MASK) | PAGE_OF_BLK(attr));
}

static void z_c7x_mmu_map(uint64_t *l0, uint64_t va, uint64_t pa, uint64_t size, uint32_t attr_idx)
{
	/* attr_idx comes from 3-bit config fields and z_c7x_mm_init programs all
	...
	while (size != 0ULL) {
		if ((size >= C7X_MMU_BLOCK_SIZE) &&
			...
			z_c7x_mmu_map_block(l0, va, pa, attr);
			va   += C7X_MMU_BLOCK_SIZE;
			pa   += C7X_MMU_BLOCK_SIZE;
			size -= C7X_MMU_BLOCK_SIZE;
		} else {
			z_c7x_mmu_map_page_l3(l0, va, pa, attr);
...
void z_c7x_mm_init(void)
{
	...
	/* MMU tables reside in outer and inner write-back cacheable memory */
	z_c7x_mmu_tcr_set(C7X_TCR_ADDR_BITS(48U) | C7X_TCR_WALK_EN |
			  C7X_TCR_MEMTYPE_CACHEABLE | C7X_TCR_OUTER_CACHEABLE |
			  C7X_TCR_INNER_CACHEABLE | C7X_TCR_OUTER_SHAREABLE);

	z_c7x_mmu_set_next_slot(0U);
	l0 = z_c7x_mmu_alloc_table();
	for (i = 0U; i < mmu_config.num_regions; i++) {
		r = &mmu_config.mmu_regions[i];
		z_c7x_mmu_map(l0, (uint64_t)r->base_va, (uint64_t)r->base_pa, (uint64_t)r->size, r->attrs);
	}
...
static int z_c7x_mmu_map_page(uint64_t *l0, uint64_t va, uint64_t pa, uint32_t attr_idx)
{
	uint32_t i0 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(0)) & C7X_MMU_INDEX_MASK);
	uint32_t i1 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(1)) & C7X_MMU_INDEX_MASK);
	uint32_t i2 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(2)) & C7X_MMU_INDEX_MASK);
	...
	l1 = z_c7x_mmu_next_table(l0, i0);
	if (l1 == (uint64_t *)0) {
		return -ENOMEM;
	}
	l2 = z_c7x_mmu_next_table(l1, i1);
	if (l2 == (uint64_t *)0) {
		return -ENOMEM;
	}
	l3 = z_c7x_mmu_next_table(l2, i2);
	if (l3 == (uint64_t *)0) {
		return ((z_c7x_mmu_read_entry(l2, i2) & C7X_MMU_DESC_TYPE_MASK) == C7X_MMU_DESC_BLOCK)
			       ? -EINVAL
			       : -ENOMEM;
	}
	if ((z_c7x_mmu_read_entry(l3, i3) & C7X_MMU_DESC_TYPE_MASK) != C7X_MMU_DESC_INVALID) {
		return -EBUSY;
	}
	z_c7x_mmu_write_entry(l3, i3, (pa & ~C7X_MMU_PAGE_MASK) | PAGE_BASE | C7X_MMU_ATTR_IDX(attr_idx));
	return 0;
}

static void z_c7x_mmu_unmap_page(uint64_t *l0, uint64_t va)
{
	uint32_t i0 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(0)) & C7X_MMU_INDEX_MASK);
	uint32_t i1 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(1)) & C7X_MMU_INDEX_MASK);
	uint32_t i2 = (uint32_t)((va >> C7X_MMU_LEVEL_SHIFT(2)) & C7X_MMU_INDEX_MASK);
	...
	l3 = (uint64_t *)(uintptr_t)(e & ~C7X_MMU_PAGE_MASK);
	z_c7x_mmu_write_entry(l3, i3, C7X_MMU_DESC_INVALID);
}
...
static int z_c7x_mem_map(void *virt, uintptr_t phys, size_t size, uint32_t flags)
{
	...
	for (off = 0U; off < size; off += C7X_MMU_PAGE_SIZE) {
		rc = z_c7x_mmu_map_page(l0, (uint64_t)(va + off),
				      (uint64_t)(phys + off), attr_idx);
		if (rc != 0) {
			break;
		}
	}
	(void)arch_dcache_flush_range((void *)z_c7x_mmu_get_tables_base(), C7X_MMU_POOL_BYTES);
	if (rc == 0) {
		z_c7x_mmu_tlb_inv(C7X_TLB_INV_ALL);
	}
	irq_unlock(key);
...
static int z_c7x_mem_unmap(void *addr, size_t size)
{
	...
	l0 = z_c7x_mmu_get_tables_base();
	for (off = 0U; off < size; off += C7X_MMU_PAGE_SIZE) {
		z_c7x_mmu_unmap_page(l0, (uint64_t)(va + off));
	}
	(void)arch_dcache_flush_range((void *)z_c7x_mmu_get_tables_base(), C7X_MMU_POOL_BYTES);
	z_c7x_mmu_tlb_inv(C7X_TLB_INV_ALL);
	irq_unlock(key);
...
{
	(void)z_c7x_mem_unmap(addr, size);
}
