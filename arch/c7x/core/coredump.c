...
/* Fixed address denoted in the linker */
extern char __c7x_diag_start[];
extern char __c7x_diag_end[];
...
void arch_coredump_info_dump(const struct arch_esf *esf)
{
	...
	coredump_memory_dump((uintptr_t)__c7x_diag_start, (uintptr_t)__c7x_diag_end);
...
uintptr_t arch_coredump_stack_ptr_get(const struct k_thread *thread)
{
	...
	/* Find the faulting thread's SP in the dumped frame.
	 * Otherwise retrieve it in its switch handle.
	 */
	if (thread == _current) {
